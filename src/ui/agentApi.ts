...
function firstSentence(text: string) {
  ...
  return sentence.length > ROW_HELP_CHARS
    ? `${sentence.slice(0, ROW_HELP_CHARS).replace(/\s+\S*$/, '')}…`
    : sentence
}
  ...
  return {
    ...
    describe(key: string) {
      ...
      if (!def) {
        ...
        throw new Error(
          group
            ? `'${key}' is a group; bender.find('${group.name}') lists its controls`
            : unknownKey(key),
        )
      }
      const c = engine.controls.get()
