...
import {
  ...
  PCM_VOICE_NAMES,
  SAMPLE_SECS,
} from '../../dsp/stages/pcmRom'
import { PET_ADDR_LINES, PET_DATA_LINES } from '../../dsp/stages/petRom'
import {
  ROM_ADDR_LINES,
  ROM_DATA_LINES,
  ROMS,
...
const fmCutSomewhere = cutOn(
  ...
  ['fmWaveDataLine', 'fmWaveDataFault'],
)
...
const pcmCutSomewhere = cutOn(
  ['pcmDataLine', 'pcmDataFault'],
  ['pcmAddrLine', 'pcmAddrFault'],
)
  ...
  },
  {
    ...
    sliders: [
      ...
      },
      {
        ...
        step: 0.01,
        unit: '',
        help: 'How loud the modulator is into the carrier, which on a two-operator chip is the whole of the tone control: down is a sine, up is the harmonics piling on. It rides out with the patch as one of those eight bytes, so moving it is another write for a fault to catch. Past 1 a booster wired around the chip multiplies the modulation index by four per unit, up to 64 times at the top, and the tone breaks up into noise. The booster does nothing while an effect runs.',
      },
      {
        key: 'fmFeedback',
        label: 'Feedback',
      ...
      },
      {
        ...
        step: 0.01,
        unit: '',
        help: 'A blob of solder from the shift register’s output onto the pins the sine table reads out on. That register is the only broadband thing on the die and the percussion bank is the only way to it — this is the way round, and it needs no mode bit, so it is there on every patch, under the effect ROM, and through a panic. A touch of it is dirt riding on the note; all the way across is eight pins holding one bit, and the sine is gone.',
      },
      {
        key: 'fmRhythm',
        label: 'Rhythm',
      ...
      },
      {
        ...
        step: 1,
        unit: '',
        choices: lineNames('T', 8),
        shy: true,
        help: 'Which of the eight wires the sine table answers on the knife found. The address side picks a different correct sample; this side corrupts the one that was picked — a step cut into the amplitude at one bit position, on every operator’s turn on the one datapath. The table keeps a magnitude and takes the sign off the phase, so nothing here moves a note.',
      },
      {
        key: 'fmWaveDataFault',
        ...
        shy: true,
        choices: FAULT_NAMES,
        help: 'The same four things, on the table’s way out. To +V is a floor under every sample, so the quiet parts of a wave stop being quiet and the note comes out buzzing at full width; to ground punches a notch through the middle of the sine. T7 is the top of the word and takes half the amplitude with it.',
      },
      {
        key: 'fmBusCut',
        part: 'knife on the bus',
    ...
    sliders: [
      {
        ...
        step: 0.01,
        unit: '',
        help: 'How loud the home keyboard is in the source mix. Like the FM chip it boots at nothing and has no sequencer of its own — its key input is soldered onto the toy’s gate line, so out of the box the demo song plays it. Cut that jumper and it answers only the keybed drawn under the toy’s. Same rail as the other two, so starving the toy drags its pitch down with everything else.',
      },
      {
        key: 'pcmVoice',
        label: 'Voice',
        min: 0,
        max: PCM_VOICE_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: PCM_VOICE_NAMES,
        help: 'Which recording in the ROM the address counter walks. None of them is a sample of anything: each is a handful of partials and an attack shape cut into eight-bit words when the chip was built, an attack segment and then a loop the counter drops back into and stays in. *sampled* is the exception and the whole reason a keyboard like this is famous — it walks whatever you dropped on the sampler instead.',
      },
      {
        key: 'pcmEnv',
        label: 'Envelope',
        min: 0,
        max: PCM_ENV_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: PCM_ENV_NAMES,
        help: 'The four buttons down the side of the case. *piano* decays to nothing whether or not you are still holding the key, so the loop is only ever heard under a fall already on its way out; *organ* holds while a key is down; *pad* swells in and out; *chime* starts at once and rings for a long time. Every rate here counts off the divider the pitch does, so a starving rail stretches all four.',
      },
      {
        key: 'pcmRelease',
        label: 'Release',
        ...
        unit: 's',
        curve: 'log',
        help: 'The one capacitor the envelope generator has that the buttons do not pick: how long a note takes to die once the key comes up. It is also what decides how long a strike off the toy’s gate or the kit’s trigger lines rings, since neither of those has a hand on it to hold the key down.',
      },
      {
        key: 'pcmTone',
        label: 'Tone',
        ...
        unit: 'Hz',
        curve: 'log',
        help: 'The reconstruction filter behind the DAC — two poles and no more, which is all a keyboard at this price ever had. The ROM was cut at 9.9 kHz and nothing interpolates on the way out, so everything above half that comes back folded over the note. Wind this up and you hear the images; wind it down and you get the dull, underwater version the same part sounds like through a cheap speaker.',
      },
      {
        key: 'pcmVibrato',
        label: 'Vibrato',
        min: 0,
        max: PCM_VIBRATO_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: PCM_VIBRATO_NAMES,
        help: 'One LFO on the die with no rate register, no depth register and nothing that can stop it — the button only decides whether a voice is wired to it. *delayed* is the same wire behind a lag, so a note arrives straight and comes into the wobble a moment later. It counts off the same divider as everything else, so a starving board slows the wobble too.',
      },
      {
        key: 'pcmChord',
        label: 'Chord memory',
        min: 0,
        max: PCM_CHORD_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: PCM_CHORD_NAMES,
        help: 'One key, a whole voicing under it. The chip has four voices, so a four-note chord is the entire instrument — press a second key and the first one is stolen out from under you, which is what a home keyboard with a chord button has always done. It reaches whatever strikes the key input, so the toy’s demo song comes out harmonised.',
      },
      {
        key: 'pcmClockX',
        label: 'ROM clock',
        ...
        unit: '×',
        curve: 'log',
        help: 'Drags the counter that walks the ROM. The classic clock bend: pitch and the aliasing move together, because on this part they are the same thing — there is no oscillator to detune, only the rate the recording is read back at. Down low the images come under the note; up high the voice runs out of ROM before the loop can settle.',
      },
      {
        key: 'pcmStruck',
        label: 'Struck by',
        ...
        unit: '',
        choices: ['off', ...VOICE_LABELS, 'any hit'],
        help: 'The kit’s trigger lines, clipped onto this keyboard’s key input. A trigger carries a strike and nothing else, so the note is the last one the keyboard played — a pattern rekeying whatever chord you left standing. Four voices, so a busy grid steals its own notes.',
      },
      {
        key: 'pcmKeyGate',
        label: 'Toy gate',
        ...
        unit: '',
        choices: ['soldered', 'cut'],
        help: 'The same jumper the FM chip has, off the toy’s gate line onto this keyboard’s key input. Out of the box the demo song and the arpeggio play the strings. Cut it and the keyboard answers only its own bed, the kit’s trigger lines and a controller. Its own keybed plays either way.',
      },
      {
        key: 'pcmAddrLine',
        lane: 'keys addr line',
        ...
        step: 1,
        unit: '',
        choices: lineNames('A', PCM_ADDR_LINES),
        shy: true,
        help: `Which of the wires the counter addresses the ROM on the knife found. Nothing here is corrupted — every word that comes back is a real word, from the wrong place. A low line held reads every other sample twice and drops an octave; a high one folds the voice onto its own first half and loops there. The top two only ever go high on the sampled voice: the counter is wider than the wave ROM, and only ${SAMPLE_SECS} seconds of sample memory fills it.`,
      },
      {
        key: 'pcmAddrFault',
        lane: 'keys addr fault',
        part: 'knife on the bus',
        needs: c => c.pcmAddrLine > 0,
        label: 'Address fault',
        min: 0,
        max: FAULT_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: FAULT_NAMES,
        help: 'The same four things a knife does to any trace. To ground and to +V nail the wire and the voice folds onto a fixed half of itself. Cut leaves the pin floating, so it holds whatever the last address had on it and then drifts after the line next door — the loop goes stale and starts wandering, which is the one of the four that never stands still.',
      },
      {
        key: 'pcmDataLine',
        lane: 'keys data line',
        ...
        step: 1,
        unit: '',
        choices: lineNames('D', PCM_DATA_LINES),
        shy: true,
        help: 'Which of the eight wires the ROM answers on the knife found — the famous one. The address is right and the word is not, on every sample of every voice, so the note keeps its pitch and loses its shape. D7 is the top of the word and carries the sign; the low wires are a floor under the quiet parts of a wave, which is what turns a choir into gravel.',
      },
      {
        key: 'pcmDataFault',
        lane: 'keys data fault',
        part: 'knife on the bus',
        needs: c => c.pcmDataLine > 0,
        label: 'Data fault',
        min: 0,
        max: FAULT_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: FAULT_NAMES,
        help: 'To +V is the bend everybody knows: that bit is set in every word the ROM hands back, so nothing the chip plays is ever quiet again and a pad comes out as a square-edged buzz at the same pitch it always was. To ground punches a notch through every word. Bridged collapses two wires onto each other, and the voice comes out in steps.',
      },
      {
        key: 'pcmBusCut',
        part: 'knife on the bus',
        needs: pcmCutSomewhere,
        label: 'Cut depth',
        min: 0,
        max: 1,
        step: 0.01,
        unit: '',
        help: 'How far through the trace the knife went — the cut fault is the only one that reads it. All the way and the pin floats free. Back it off and the line conducts some of the time, so most words arrive and the rest come off the wire next door: a voice that flickers between itself and something else, at the rate the ROM is being read.',
      },
    ],
  },
  {
    ...
    sliders: [
      {
        ...
        step: 0.01,
        unit: '×',
        help: 'Level of the dropped audio file, looping through the chain. ×1 plays the file at its recorded level, and higher settings add gain.',
      },
      {
        key: 'sampleSpeed',
      ...
      },
      {
        ...
        step: 0.01,
        unit: '',
        help: 'How much of the board’s own output the record head lays back down, on the spot the play head is reading — live, onto whatever is already on the reel, with nothing in the app to put it back once it has gone under the head. Every other knob here only changes what you hear; this one rewrites what is on the tape. Which makes this a tape loop rather than a sampler: what comes round next lap has been through the whole board, and then goes through it again.',
      },
      {
        key: 'loopErase',
        label: 'Erase',
        ...
        step: 0.01,
        unit: '',
        help: 'How much of what is already on the tape the erase head takes off on the way past, permanently, and only while Record above is doing anything at all. All the way up, each lap replaces the last — a delay the length of the reel. All the way down the head is disconnected and the laps pile up on top of what was there until the oxide runs out of room.',
      },
      {
        key: 'loopSecs',
        label: 'Reel length',
        ...
        step: 0.25,
        unit: 's',
        help: 'How long a blank reel to thread when you arm the record head with nothing loaded — the one setting here that never touches a recording that already exists. Read once, when the tape goes on: a reel you have already recorded on is the length it is, and a knob that resized it would be throwing the take away. Drop a file in and the file is the reel.',
      },
    ],
  },
  {
    ...
    sliders: [
      {
        ...
        step: 0.01,
        unit: '',
        help: 'How loud the talking pet is in the source mix. The pet is a furry toy on the same batteries as the keyboard: a speech chip, a phrase ROM and a cam motor that moves its eyes and ears. Turn it up and it says hello.',
      },
      {
        key: 'petMotor',
        label: 'Motor',
        ...
        step: 0.01,
        unit: '',
        help: 'How much of the cam motor you hear: a brush buzz and a gear click that run while the pet talks or blinks. The knob sets the sound only. The motor draws current from the shared supply whenever it turns, so a talking pet sags the keyboard, the kit and the FM chip.',
      },
      {
        key: 'petPitch',
        lane: 'pet pitch',
        ...
        unit: '×',
        curve: 'log',
        help: 'Scales the pitch period the frames ask for, so the voice goes up or down while the formants stay put. Mood nudges it too: a scared pet squeaks and a sleepy one drops.',
      },
      {
        key: 'petRate',
        lane: 'pet rate',
        ...
        unit: '×',
        curve: 'log',
        help: 'The speech chip’s 8 kHz clock. Pitch, formants and the 25 ms frame rate all move together, so fast is a chipmunk and slow is a slur. The clock also follows the toy supply, so a starved rail drags the pet down with the keyboard.',
      },
      {
        key: 'petChatter',
        label: 'Chatter',
      ...
      },
      {
        ...
        step: 0.01,
        unit: '',
        help: 'The chance at each 25 ms frame that the decoder keeps the frame it has and reads nothing new. A little stutters a syllable; all the way up freezes one vowel until the phrase times out. A latched supply holds every frame.',
      },
      {
        key: 'petKBits',
        label: 'K bits',
        ...
        step: 0.01,
        unit: '',
        help: 'Flips bits in the latch that holds the ten reflection coefficients of the lattice filter, a chance per coefficient per frame. A low bit shifts a formant. The top bits push a coefficient past one, the lattice goes unstable, and the saturating adders clamp it into a screech.',
      },
      {
        key: 'petAddrLine',
        lane: 'pet addr line',
        ...
        step: 1,
        unit: '',
        choices: lineNames('A', PET_ADDR_LINES),
        shy: true,
        help: 'Which of the twelve wires addressing the phrase ROM the knife found. The decoder reads a phrase as a stream of bits, so a wrong address lands it in the middle of another word: skips, stutters, and frames read from the wrong place, which sets garbage coefficients.',
      },
      {
        key: 'petAddrFault',
        lane: 'pet addr fault',
        part: 'knife on the bus',
        needs: c => c.petAddrLine > 0,
        label: 'Address fault',
        min: 0,
        max: FAULT_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: FAULT_NAMES,
        help: 'What happened to the wire. A low line held to ground reads each even byte twice and skips the odd ones; a high line throws the read into another phrase altogether. The ROM fills the bottom of the address space and repeats above it.',
      },
      {
        key: 'petDataLine',
        lane: 'pet data line',
        ...
        step: 1,
        unit: '',
        choices: lineNames('D', PET_DATA_LINES),
        shy: true,
        help: 'Which of the eight wires the ROM answers on the knife found. Every byte comes back with that bit wrong, so energies, pitches and coefficient codes change, and a stop code can arrive early and cut a word off.',
      },
      {
        key: 'petDataFault',
        lane: 'pet data fault',
        part: 'knife on the bus',
        needs: c => c.petDataLine > 0,
        label: 'Data fault',
        min: 0,
        max: FAULT_NAMES.length - 1,
        step: 1,
        unit: '',
        choices: FAULT_NAMES,
        help: 'The same four things on the data side. To +V on a high line fills the stream with loud frames and stop codes; to ground drops bits out and the pet mumbles. The codes still pick entries out of the coefficient tables, so the extremes ring sharp and hard.',
      },
    ],
  },
