...
const BLURBS: Record<string, string> = {
  ...
  'Home keyboard':
    'The eight-bit sampling home keyboard, and the only thing on this board that makes pads, strings, choirs and bells. There is no oscillator in it: a ROM holds one short recording of each voice — an attack and then a loop — and a counter walks that ROM at whatever rate the note asks for, which is the whole architecture and the whole reason it is the most bent instrument ever built. The ROM was cut at 9.9 kHz with nothing interpolating on the way out, so every voice comes back with its own images over it and *Tone* is the two-pole reconstruction filter that decides how much of that you hear. The rate is a twelve-bit divider rather than a frequency, so the top octave arrives a few cents sharp and the bottom is dead in tune. *Address line* is the counter’s own wires — a real word from the wrong place, so a voice folds onto its own first half or reads every sample twice — and *Data line* is the famous one: the top wire held high and the bottom half of every wave folds up onto the top of it. *Chord memory* puts a voicing under one key and fills all four voices doing it, and *sampled* plays whatever you dropped on the sampler across the keyboard, out of a second and a half of memory with the reel’s own loop markers as its loop.',
  'Chaos osc':
  ...
  'Talking pet':
    'A furry talking toy on the same batteries as the keyboard: an LPC speech chip with a 10-stage lattice filter, a phrase ROM of made-up pet words, and a cam motor for the eyes and ears. A mood machine picks what it says: sound wakes it, a loud sound scares it, kit hits tickle it, quiet and a low supply send it to sleep. The motor loads the shared rail, so a talking pet sags the toys beside it. The knife reaches the ROM’s address and data lines, *Frame hold* stutters the frames, and *K bits* flips coefficient bits until the lattice screeches against its clamp.',
  ...
  'Tape delay':
    'The capstan is a real motor: it has weight, it answers the brake slowly, and *Supply drag* wires it to the same dying rail as the toy, so the repeats dive in pitch as the board browns out. Three play heads sit along the tape at one, two and three spacings, and *Heads* is which of them are up. The tape is a five-second loop joined once: *Splice* is how bad the join is, and *Erase* how much of the last lap the erase head lets through under the new one.',
  'Delay pedal':
...
function ends(s: SliderDef, min: number, max: number): string {
  const { unit } = s
  const signed = s.min < 0
  ...
  if (!unit) {
    if (min === 0 && max === 1 && !s.normal) return 'off to full'
